...
const ProfileView = ({ onBack }) => {
    // Demo data - Sample scans in prototype mode
    const recentScans = [
        { id: '00001', date: '2024-01-15', type: 'Chest X-Ray (PA)', status: 'Completed' },
        { id: '00002', date: '2024-01-14', type: 'Chest X-Ray (PA)', status: 'Under Review' },
        { id: '00003', date: '2024-01-12', type: 'Chest X-Ray (PA)', status: 'Completed' },
    ];
    ...
    return (
        <div className="w-full h-full p-8 overflow-y-auto bg-[var(--bg-dark)]">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                {/* Stats Grid - Prototype Warning */}
                <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-4 flex items-center gap-3">
                    <span className="text-yellow-500 text-2xl">⚠️</span>
                    <div>
                        <p className="text-yellow-300 font-medium">Prototype Mode</p>
                        <p className="text-yellow-500/80 text-sm">Statistics below are for demo purposes. Real data is not connected.</p>
                    </div>
                </div>

                <div className="grid grid-cols-3 gap-4">
                    <div className="card p-6 border-l-4 border-[var(--primary)]">
                        <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider block mb-1">Total Analysis</span>
                    ...
                    <div className="card p-6 border-l-4 border-[var(--success)]">
                        <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider block mb-1">Model Type</span>
                        <span className="text-xl font-bold">Multi-Label</span>
                        <span className="text-xs text-[var(--text-muted)]">14 Disease Classes</span>
                    </div>
                    <div className="card p-6 border-l-4 border-[var(--accent)]">
                        <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider block mb-1">Dataset</span>
                        <span className="text-xl font-bold">NIH Chest X-ray</span>
                        <span className="text-xs text-[var(--text-muted)]">~112,000 images</span>
                    </div>
                </div>
                ...
                <div className="card overflow-hidden border border-[var(--border)]">
                            ...
                            <tbody>
                                {recentScans.map((scan, i) => (
                                    <tr key={i} className="border-b border-[var(--border)] hover:bg-[var(--bg-card-hover)] transition group">
                                        ...
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded text-xs border ${scan.status === 'Completed' ? 'bg-green-900/20 text-green-300 border-green-900' :
                                                scan.status === 'Under Review' ? 'bg-yellow-900/20 text-yellow-300 border-yellow-900' :
                    ...
                    <div className="p-4 text-center border-t border-[var(--border)]">
                        <button className="text-sm text-[var(--text-muted)] hover:text-white transition">View All History</button>
                    </div>
                </div>
